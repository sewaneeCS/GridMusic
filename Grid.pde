...
public class Grid {
  ...
  Sequence seq;
  float baseDuration;
  // initialize with position, something, scale, duration represented by grid position, and total seed duration 
  public Grid(PVector pos, PVector s, Scale sc, float baseDur, int tDur) {
    this.pos=pos;
    this.s=s;
  ...
  // update state of the Grid (new0 scale, base duration, and total seed duration
  ...
  // initGrid - create a Block {currPos, (7,4), notes, baseDuration} for each Grid pos
  private void initGrid() {
    PVector SIZE = new PVector(9, 6);
